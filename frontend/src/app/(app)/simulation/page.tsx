...
function genAlgoPerformance(density: number, weather: Weather) {
  ...
  return [
    { algo: "Dijkstra", latency: +(rand(12, 22) * loadFactor / wf).toFixed(1), nodes: Math.round(rand(42000, 78000) * loadFactor) },
    { algo: "A*", latency: +(rand(4, 10) * loadFactor / wf).toFixed(1), nodes: Math.round(rand(18000, 35000) * loadFactor) },
    { algo: "Bi-A*", latency: +(rand(2, 6) * loadFactor / wf).toFixed(1), nodes: Math.round(rand(8000, 18000) * loadFactor) },
  ];
}
...
export default function SimulationPage() {
  ...
  const regenerate = useCallback(() => {
    ...
    setKpis({
      ...
      sims: simCount,
      sync: running ? "LIVE" : "SYNCED",
    });
  }, [density, weather, roadClosures, running, simCount]);
    ...
    <div className="h-full w-full flex bg-[#0a0a0a] overflow-hidden relative">
      ...
      <aside className="w-80 shrink-0 border-r border-border bg-card/50 backdrop-blur-xl p-6 flex flex-col gap-6 z-10 overflow-y-auto">
        ...
        <ControlGroup label="Traffic Density" icon={<Car className="w-4 h-4 text-amber-400" />} valueLabel={`${density}%`}>
          <input type="range" min={0} max={100} value={density} onChange={(e) => setDensity(+e.target.value)} className="w-full accent-emerald-500 cursor-pointer" />
        ...
        <ControlGroup label="Weather Impact" icon={<Cloud className="w-4 h-4 text-sky-400" />} valueLabel={weather}>
          <select value={weather} onChange={(e) => setWeather(e.target.value as Weather)} className="w-full bg-secondary/60 border border-border rounded-md px-3 py-2 text-sm font-mono focus:outline-none focus:ring-1 focus:ring-emerald-500/60 appearance-none cursor-pointer">
        ...
        <ControlGroup label="Road Closures" icon={<Activity className="w-4 h-4 text-rose-400" />} valueLabel={roadClosures ? "Active" : "Inactive"}>
          <button onClick={() => setRoadClosures((v) => !v)} className={`relative w-12 h-6 rounded-full transition-colors duration-200 ${roadClosures ? "bg-emerald-500" : "bg-secondary"}`}>
        ...
        <ControlGroup label="Time of Day" icon={<Clock className="w-4 h-4 text-violet-400" />} valueLabel={`${String(timeOfDay).padStart(2, "0")}:00`}>
          <input type="range" min={0} max={24} value={timeOfDay} onChange={(e) => setTimeOfDay(+e.target.value)} className="w-full accent-emerald-500 cursor-pointer" />
        ...
        {/* Run Simulation */}
        <button onClick={handleRunSimulation} className={`w-full flex items-center justify-center gap-2 py-3 rounded-lg font-semibold text-sm transition-all duration-200 ${running ? "bg-rose-500/20 border border-rose-500/40 text-rose-400 hover:bg-rose-500/30" : "bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 hover:bg-emerald-500/30"}`}>
        ...
        <div className="mt-auto pt-4 border-t border-border text-xs text-muted-foreground font-mono space-y-2">
          <div className="flex items-center justify-between"><span>Engine</span><span className="text-emerald-400">v4.2.1 Online</span></div>
          <div className="flex items-center justify-between"><span>Twin Sync</span><span className={running ? "text-sky-400 animate-pulse" : "text-emerald-400"}>{kpis.sync}</span></div>
      ...
      <main className="flex-1 overflow-y-auto p-8 z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
          <KpiCard icon={<Clock className="w-4 h-4" />} label="Estimated Travel Time" value={kpis.eta} accent="text-emerald-400" />
          <KpiCard icon={<Thermometer className="w-4 h-4" />} label="Network Load" value={`${kpis.load}%`} accent={kpis.load > 80 ? "text-rose-400" : kpis.load > 55 ? "text-amber-400" : "text-emerald-400"} />
          <KpiCard icon={<Activity className="w-4 h-4" />} label="Active Simulations" value={String(simCount)} accent="text-sky-400" />
          <KpiCard icon={<Zap className="w-4 h-4" />} label="Digital Twin Sync" value={kpis.sync} accent={running ? "text-sky-400 animate-pulse" : "text-emerald-400"} />
        </div>

        {/* Charts Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/*  Travel Time vs Density  */}
          <ChartCard title="Travel Time vs Traffic Density" subtitle="Minutes by density level" icon={<Car className="w-4 h-4 text-emerald-500" />}>
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={travelData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <Line type="monotone" dataKey="time" stroke="#10b981" strokeWidth={2} dot={{ r: 3, fill: "#10b981" }} activeDot={{ r: 5 }} animationDuration={600} />
              </LineChart>
            </ResponsiveContainer>
          </ChartCard>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={algoData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <Bar dataKey="latency" radius={[4, 4, 0, 0]} animationDuration={600}>
                  {algoData.map((entry) => (
                    <Cell key={entry.algo} fill={ALGO_COLORS[entry.algo] ?? "#10b981"} fillOpacity={0.85} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <ChartCard title="Weather Impact on Speed" subtitle="km/h over distance under each condition" icon={<Cloud className="w-4 h-4 text-sky-400" />}>
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={weatherData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <Area type="monotone" dataKey="Clear" stroke="#10b981" fill="#10b981" fillOpacity={0.15} strokeWidth={2} animationDuration={600} />
                <Area type="monotone" dataKey="Rain" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.1} strokeWidth={2} animationDuration={600} />
                <Area type="monotone" dataKey="Snow" stroke="#8b5cf6" fill="#8b5cf6" fillOpacity={0.1} strokeWidth={2} animationDuration={600} />
                <Area type="monotone" dataKey="Fog" stroke="#f59e0b" fill="#f59e0b" fillOpacity={0.1} strokeWidth={2} animationDuration={600} />
              </AreaChart>
            </ResponsiveContainer>
          </ChartCard>
          ...
          <ChartCard title="Congestion Heatmap Timeline" subtitle="24-hour congestion % by road type" icon={<Activity className="w-4 h-4 text-rose-400" />}>
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={congestionData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <Line type="monotone" dataKey="Urban" stroke="#f59e0b" strokeWidth={2} dot={false} animationDuration={600} />
                <Line type="monotone" dataKey="Residential" stroke="#10b981" strokeWidth={2} dot={false} animationDuration={600} />
              </LineChart>
            </ResponsiveContainer>
          </ChartCard>
        </div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-mono text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded">{valueLabel}</span>
      </div>
      {children}
    </div>
  );
...
function ChartCard({ title, subtitle, icon, children }: { title: string; subtitle: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="border border-border bg-card/50 backdrop-blur rounded-lg p-5 flex flex-col">

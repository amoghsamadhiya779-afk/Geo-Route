...
export default function KnowledgeGraphPage() {
  ...
  const filteredNodes = useMemo(() => {
    return nodes.map((node) => {
      ...
      const passesFilter =
        (node.data as any).congestion >= congestionFilter;
    ...
    setNodes((nds) =>
      nds.map((n) => {
        if (n.id === nodeId) {
          const updatedData = {
            ...(n.data as any),
            congestion: Math.max(0.1, (n.data as any).congestion - 0.3),
            avgDelayMs: Math.max(20, Math.floor((n.data as any).avgDelayMs * 0.4)),
          };
      ...
      eds.map((e) => {
        if (e.source === nodeId || e.target === nodeId) {
          const updatedEdgeData = {
            ...(e.data as any),
            trafficFactor: Math.max(1.0, (e.data as any).trafficFactor - 0.5),
            currentSpeedKmh: Math.min(
              (e.data as any).baseSpeedKmh,
              Math.floor((e.data as any).currentSpeedKmh * 1.5)
            ),
          };
            ...
            <div className="grid grid-cols-2 gap-2 text-xs font-mono">
              ...
              <div className="border border-border p-2.5 rounded bg-secondary/10">
                ...
                <div className="text-lg font-bold text-rose-400">
                  {nodes.filter((n) => (n.data as any).congestion > 0.6).length}
                </div>
              </div>
              <div className="border border-border p-2.5 rounded bg-secondary/10">
